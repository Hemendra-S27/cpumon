...
#
# Usage:
#   backup_orahome.sh --OraHome <directory_to_backup> --BkpLoc <backup_location>
#
# Options:
...
#
# Example:
#   backup_orahome.sh --OraHome /home/user/mydir --BkpLoc /mnt/backup
#
#!/bin/bash
...
usage() {
  LOG_ERROR $LINENO "Invalid usage. Usage: $0 --OraHome <directory_to_backup> --BkpLoc <backup_location>"
  exit 1
}
...
if backup_orahome "$ORA_HOME" "$BKP_LOC"; then
  LOG_MESSAGE "$ORA_HOME  backup completed successfully."
  LOG_RESULT "SUCCESSFUL"
  exit 0
else
  LOG_ERROR $LINENO "$ORA_HOME  backup failed."
