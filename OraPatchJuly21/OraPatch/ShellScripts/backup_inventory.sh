...
#
# Usage:
#   backup_inventory.sh --OraInv <directory_to_backup> --BkpLoc <backup_location>
#
# Options:
#   -OraInv   Directory to backup.
#   -BkpLoc   Location where the backup should be stored.
#
# Example:
#   backup_inventory.sh --OraInv /home/user/mydir --BkpLoc /mnt/backup
#
#!/bin/bash
...
usage() {
  LOG_ERROR $LINENO "Invalid usage. Usage: $0 --OraInv <directory_to_backup> --BkpLoc <backup_location>"
  exit 1
}
...
LOG_MESSAGE "Starting $ORA_INV  backup"
if backup_inventory "$ORA_INV" "$BKP_LOC"; then
  LOG_MESSAGE "$ORA_INV  backup completed successfully."
  LOG_RESULT "SUCCESSFUL"
  exit 0
else
  LOG_ERROR $LINENO "$ORA_INV  backup failed."
